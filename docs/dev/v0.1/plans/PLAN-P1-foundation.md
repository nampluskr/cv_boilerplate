...
| 2026-08-19 | `src/core/engine.py` §7.1 | A | `Trainer.fit()`이 best-checkpoint 재로드를 `adapter.on_fit_end()` **이후**에 수행하던 순서가 버그였음을 P5 구현 중 발견. `on_fit_end`에서 모델 버퍼(예: quantile 정규화 상수)를 계산해도 직후 재로드가 그 계산을 즉시 덮어써 폐기했고, 애초에 계산 자체도 (best가 아닌) 마지막 epoch 가중치를 대상으로 이루어졌음. 순서를 "best 재로드 → `on_fit_end` → best.pth의 `model_state`만 재저장"으로 바로잡음. `optimizer_state`/`scheduler_state`/`rng_state`는 best epoch 저장 시점 값을 그대로 유지해 `--resume` 정합성을 보존. `on_fit_end`가 no-op인 태스크(P2~P4)는 동작 불변 — toy 4종 재실행과 classification/segmentation/detection 1-epoch 재실행으로 확인 | P5 task 세션(master 겸임) | master (2026-08-19) |
| 2026-08-19 | `src/core/adapter.py`, `src/cli/commands.py` | B | `TaskAdapter`에 `extra_final_metrics()` 훅(기본값 `{}`) 추가. `PLAN-P5 §6`이 요구하는 "threshold를 `metrics_final.json`에 저장"이 어댑터 금지 사항(`§6.7`, 로그·체크포인트 파일 직접 접근 금지)과 충돌해 어댑터 단독으로는 구현 불가능했음. `commands.py::train()`이 `best` 저장 직후 이 훅의 반환값을 `metrics_final.json`에 병합하도록 배선. 기본값이 빈 dict이므로 P2~P4는 파일 내용 불변 — toy 4종 재실행으로 확인 | P5 task 세션(master 겸임) | master (2026-08-19) |
| 2026-08-19 | `src/bench/runner.py` | A | P6.1 재실행 중 발견: `execute_split()`이 `cli/commands.py::evaluate()`와 달리 `output.save_visualizations` 설정일 때 `adapter.visualize()`를 호출하지 않아 벤치마크 산출물에 `visualizations/`가 전혀 생성되지 않았음(계약 무변경, 기존 CLI 경로의 로직을 벤치마크 경로에 동일하게 배선). `test_loader`의 첫 배치로 `predict_step()` + `visualize()`를 `model.eval()`/`torch.no_grad()` 하에 호출하도록 추가. 4개 벤치마크(12 split) 재실행으로 `visualizations/` 생성과 기존 지표 불변을 확인 | master (P6.1) | master (2026-08-19) |
| 2026-08-19 | `src/bench/runner.py` | A | P6.1 재실행 중 발견: `execute_split()`이 `cli/commands.py::train()`과 달리 `adapter.extra_final_metrics()`를 호출하지 않아 벤치마크의 `metrics_final.json`에 Anomaly의 `image_threshold`/`pixel_threshold`(valid-only 결정, `PLAN-P5 §6`)가 기록되지 않았음(계약 무변경, 기존 훅을 벤치마크 경로에 배선). `metrics_final.json` 저장 직전 이 훅의 반환값을 병합하도록 추가. 기본값이 빈 dict이므로 P2~P4 출력 불변 — 4개 벤치마크 재실행으로 확인 | master (P6.1) | master (2026-08-19) |
| 2026-08-19 | `src/core/builders.py::build_optimizer()` | A | P6.6 Codex 적대적 검증(Major #2)에서 지적: 고정 서브모듈(teacher)을 가진 모델도 `model.parameters()`를 필터 없이 그대로 optimizer에 전달해, `requires_grad=False`인 teacher 파라미터가 gradient 갱신은 받지 않지만(grad가 항상 `None`이라 PyTorch가 step에서 건너뜀) `optimizer.param_groups`에는 여전히 포함되어 있었음. `p.requires_grad`로 필터링하도록 수정. 학습 대상 파라미터 집합은 불변이므로(필터링 전에도 teacher는 갱신되지 않았음) 계약 무변경 — toy 4종 bit-exact 재검증과 4개 벤치마크 재실행으로 지표 불변 확인 | master (P6.6 반영) | master (2026-08-19) |
| 2026-08-19 | `src/core/offline.py` | A | P6.6 Codex 적대적 검증(Major #3)에서 지적: 오프라인 가드가 `socket.connect`/`connect_ex`/`getaddrinfo`만 패치해, 미연결(unconnected) UDP 소켓의 `sendto()`/`sendmsg()`(숫자 IP 목적지, DNS 조회 없이 `connect()`도 거치지 않음)로 패킷을 내보낼 수 있었음. `guarded_sendto`/`guarded_sendmsg`를 추가해 `AF_UNIX`가 아니고 목적지가 loopback이 아니면 `OfflineViolationError`를 던지도록 확장. 역검증(`sock.sendto(b'x', ('8.8.8.8', 53))` 차단 확인)으로 재검증 | master (P6.6 반영) | master (2026-08-19) |
| 2026-08-19 | `src/cli/commands.py::train()`, `src/bench/runner.py::execute_split()` | A | P6.6 Codex 적대적 검증(Major #1)에서 지적: `Trainer.fit()`이 반환하는 `best_metric`은 best-checkpoint 재로드 **이전**(즉 `on_fit_end`가 스코어링 버퍼를 재계산하기 이전) epoch에서 측정된 값인데, `metrics_final.json`의 `valid`에는 이 값을 그대로 기록해 `on_fit_end`가 모델 버퍼를 변경하는 태스크(Anomaly)에서 기록된 valid 지표가 실제 최종(calibrated) 체크포인트와 불일치했음(예: EfficientAD 벤치마크 재실행에서 `image_auroc` 0.805 → 0.610으로 정정). `fit()` 직후 `checkpoint_dir` 없는 1회용 `Trainer`로 valid를 재평가해 그 결과를 `metrics_final.json`에 기록하도록 변경(`metrics_epoch.csv`에는 추가 행이 남지 않음). `on_fit_end`가 no-op인 태스크는 재평가 결과가 기존 `best_metric`과 항상 일치함을 toy 4종 재검증으로 확인 | master (P6.6 반영) | master (2026-08-19) |
